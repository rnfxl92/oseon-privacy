...
시행일: 2026년 9월 24일 (푸시 알림 추가로 개정)

Oseon(이하 "앱")은 iPad와 iPhone용 악보 앱입니다. 개발자는 앱을 쓰는 사람에 관한 어떤 정보도 수집하거나 보관하지 않습니다.

**개발자가 수집하는 정보: 없음.** 앱에는 개발자가 운영하는 서버, 계정, 광고, 분석 도구, 추적 기능이 없습니다. 아래의 푸시 알림만 제3자 서비스(Google)를 거칩니다.
...
Effective date: September 24, 2026 (revised for push notifications)

Oseon ("the app") is a sheet-music app for iPad and iPhone. The developer does not collect or keep any information about the people who use it.

**Information the developer collects: none.** The app has no developer-run server, no accounts, no ads, no analytics and no tracking. Only the push notifications described below go through a third-party service (Google).
